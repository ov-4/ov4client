...
import dev.ov4client.addon.mixins.IInteractEntityC2SPacket;
import dev.ov4client.addon.modules.misc.Suicide;
import dev.ov4client.addon.utils.ov4Utils;
import dev.ov4client.addon.utils.SettingUtils;
import dev.ov4client.addon.utils.entity.ov4EntityUtils;
import dev.ov4client.addon.utils.misc.ExtrapolationUtils;
import dev.ov4client.addon.utils.player.DamageInfo;
...
public class AutoCrystalPlus extends ov4Module {
    public AutoCrystalPlus() {
        super(ov4client.Combat, "Auto Crystal+", "Automatically place and attack crystals in strict anti-cheat servers.");
    }
    ...
    private void onSend(PacketEvent.Send event) {
        if (mc.player != null && mc.world != null) {
            ...
            if (event.packet instanceof PlayerInteractBlockC2SPacket packet) {
                ...
                blocked.add(ov4Utils.getCrystalBox(packet.getBlockHitResult().getBlockPos().up()));
    ...
    private void update() {
        ...
        switch (switchMode.get()) {
            ...
            case Gapple -> {
                int gapSlot = InvUtils.findInHotbar(ov4Utils::isGapple).slot();
                if (mc.options.useKey.isPressed() && Managers.HOLDING.isHolding(Items.END_CRYSTAL, Items.ENCHANTED_GOLDEN_APPLE, Items.GOLDEN_APPLE) && gapSlot >= 0) {
                    if (getHand(ov4Utils::isGapple) == null) {
    ...
    private void explode(int id, Vec3d vec) {
        attackEntity(id, ov4Utils.getCrystalBox(vec), vec);
    }
        ...
        for (int x = -r; x <= r; x++) {
            for (int y = -r; y <= r; y++) {
                for (int z = -r; z <= r; z++) {
                    ...
                    if (ov4EntityUtils.intersectsWithEntity(box, this::validForIntersect, extHitbox)) continue;
