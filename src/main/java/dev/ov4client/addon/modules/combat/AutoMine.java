...
import dev.ov4client.addon.modules.settings.SwingSettings;
...
public class AutoMine extends ov4Module {
    public AutoMine() {
        super(ov4client.Combat, "Auto Mine", "Automatically mines blocks to destroy your enemies.");
    }
    ...
    private void onRender(Render3DEvent event) {
        ...
        if (target != null) {
            if (lastState != null && target.pos.equals(lastPos) && target.manual && manualRemine.get() && !fastRemine.get() && !lastState.isSolid() && ov4Utils.solid2(target.pos)) {
                started = false;
            }
    ...
    private void update() {
        ...
        if (!ov4Utils.solid2(target.pos)) return;
    ...
    private Target getCev() {
        ...
        for (AbstractClientPlayerEntity player : enemies) {
            ...
            if (!SettingUtils.inAttackRange(ov4Utils.getCrystalBox(pos.up()))) {
                continue;
            }
    ...
    private Target getSurroundMiner() {
        ...
        for (AbstractClientPlayerEntity player : enemies) {
            for (Direction dir : Direction.Type.HORIZONTAL) {
                ...
                if (((!civ || !pos.equals(civPos)) && !ov4Utils.solid2(pos)) || getBlock(pos) == Blocks.BEDROCK) {
                    continue;
                }
    ...
    private Target getAntiBurrow() {
        ...
        for (AbstractClientPlayerEntity player : enemies) {
            ...
            if (!ov4Utils.solid2(pos) || getBlock(pos) == Blocks.BEDROCK) {
                continue;
            }
