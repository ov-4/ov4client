...
import dev.ov4client.addon.utils.timers.TimerList;
...
public class AutoTrapPlus extends ov4Module {
    public AutoTrapPlus() {
        super(ov4client.Combat, "Auto Trap Plus", "Traps people in a box to prevent them from moving.");
    }
    ...
    private List<BlockPos> getValid(List<BlockPos> blocks) {
        ...
        blocks.forEach(block -> {
            if (!ov4Utils.replaceable(block)) return;
            ...
            for (Direction dir : Direction.values()) {
                if (!ov4Utils.replaceable(block.offset(dir)) || !SettingUtils.inPlaceRange(block.offset(dir))) {
                    continue;
                }
    ...
    private List<BlockPos> getBlocks(PlayerEntity player, int[] size, boolean higher) {
        ...
        for (int x = size[0] - 1; x <= size[1] + 1; x++) {
            for (int z = size[2] - 1; z <= size[3] + 1; z++) {
                ...
                boolean ignore = isX && !isZ ? (!ov4Utils.replaceable(pos.add(ov4Utils.closerToZero(x), 0, z)) || placed.contains(pos.add(ov4Utils.closerToZero(x), 0, z))) :
                    !isX && isZ && (!ov4Utils.replaceable(pos.add(x, 0, ov4Utils.closerToZero(z))) || placed.contains(pos.add(x, 0, ov4Utils.closerToZero(z))));

                BlockPos bPos = null;

                if (eye() && isX != isZ && !ignore) {
                    bPos = new BlockPos(x, pos.getY(), z).add(pos.getX(), 0, pos.getZ());
                } else if (top() && !isX && !isZ && ov4Utils.replaceable(pos.add(x, 0, z)) && !placed.contains(pos.add(x, 0, z)))
