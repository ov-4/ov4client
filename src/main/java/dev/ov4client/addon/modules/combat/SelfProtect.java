...
public class SelfProtect extends ov4Module {
    public SelfProtect() {
        super(ov4client.Combat, "Self Protect", "Covers you if any enemy tries to attack you.");
    }
    ...
    private void onBlock(BlockUpdateEvent event) {
        if (event.oldState.getBlock() != event.newState.getBlock() && !ov4Utils.replaceable(event.pos) && placePositions.contains(event.pos)) {
    ...
    private void onRender(Render3DEvent event) {
        ...
        mining.removeIf(m -> System.currentTimeMillis() > m.time + maxMineTime.get() * 1000 || (mineStart != null && m.id == mineStart.id) || !ov4Utils.solid2(m.pos));
    ...
    private boolean shouldProtect(ProtectBlock p) {
        ...
        switch (p.type) {
            case 1 -> {
                if (!ov4Utils.solid2(pos) || mc.world.getBlockState(pos).getBlock() == Blocks.BEDROCK) return false;
            }
            case 2, 3 -> {
