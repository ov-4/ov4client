...
public class BedBombV4 extends ov4Module {
    public BedBombV4() {
        super(ov4client.Combat, "Bed Bomb V4", "Automatically places and breaks beds to cause damage to your opponents in strict anticheat servers.");
    }
    ...
    private void calculate(int index) {
        ...
        for (int i = lastIndex; i < index; i++) {
            ...
            for (Direction dir : Direction.Type.HORIZONTAL) {
                ...
                if (!ov4Utils.replaceable(pos.offset(dir)) && !(mc.world.getBlockState(pos.offset(dir)).getBlock() instanceof BedBlock))
                    continue;
        ...
        for (int x = -i; x <= i; x++) {
            for (int y = -i; y <= i; y++) {
                for (int z = -i; z <= i; z++) {
                    ...
                    if (!ov4Utils.replaceable(pos) && !(mc.world.getBlockState(pos).getBlock() instanceof BedBlock))
                        continue;
    ...
    private void update() {
        ...
        if (logicMode.get() == LogicMode.PlaceBreak) {
            ...
            if (ov4Utils.replaceable(placePos) && ov4Utils.replaceable(placePos.offset(bedDir)) && placeUpdate()) {
                removeBed2(placePos);
