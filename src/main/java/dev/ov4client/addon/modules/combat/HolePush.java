...
import dev.ov4client.addon.enums.HoleType;
...
import dev.ov4client.addon.utils.player.PlaceData;
import dev.ov4client.addon.utils.world.hole.HoleUtils;
...
public class HolePush extends ov4Module {
    public HolePush() {
        super(ov4client.Combat, "Hole Push", "Automatically pushes people out of their safe holes.");
    }
    ...
    private void update() {
        ...
        for (AbstractClientPlayerEntity player : mc.world.getPlayers()) {
            ...
            if (!ov4Utils.solid2(player.getBlockPos()) && onlyHole.get() && HoleUtils.getHole(player.getBlockPos(), true, true, false, 1, true).type == HoleType.NotHole) return;
    ...
    private void updatePos(PlayerEntity player) {
        ...
        if (ov4Utils.solid2(eyePos.up())) return;
        ...
        for (Direction dir : Direction.Type.HORIZONTAL.stream().sorted(Comparator.comparingDouble(d -> eyePos.offset(d).toCenterPos().distanceTo(mc.player.getEyePos()))).toList()) {
            ...
            if (!ov4Utils.replaceable(pos) && !(mc.world.getBlockState(pos).getBlock() instanceof PistonBlock) && mc.world.getBlockState(pos).getBlock() != Blocks.MOVING_PISTON) continue;
            if (ov4Utils.solid2(eyePos.offset(dir.getOpposite()))) continue;
            if (ov4Utils.solid2(eyePos.offset(dir.getOpposite()).up())) continue;
            if (!ov4Utils.solid2(eyePos.offset(dir.getOpposite()).down())) continue;
    ...
    private void updateRedstone(BlockPos pos) {
        if (redstone.get() == Redstone.Torch) {
            for (Direction direction : Arrays.stream(Direction.values()).sorted(Comparator.comparingDouble(i -> pos.offset(i).toCenterPos().distanceTo(mc.player.getEyePos()))).toList()) {
                ...
                if (!ov4Utils.replaceable(position) && !(mc.world.getBlockState(position).getBlock() instanceof RedstoneTorchBlock)) {
                    continue;
                }

                redstoneData = SettingUtils.getPlaceDataAND(position, d -> {
                    if (d == Direction.UP && !ov4Utils.solid(position.down())) {
                        return false;
                    }
        ...
        for (Direction direction : Arrays.stream(Direction.values()).sorted(Comparator.comparingDouble(i -> pos.offset(i).toCenterPos().distanceTo(mc.player.getEyePos()))).toList()) {
            ...
            if (!ov4Utils.replaceable(position) && mc.world.getBlockState(position).getBlock() != Blocks.REDSTONE_BLOCK) {
                continue;
            }
