...
public class OffHandPlus extends ov4Module {
    public OffHandPlus() {
        super(ov4client.Combat, "OffHand+", "Better offhand.");
    }
    ...
    private Predicate<Item> getPredicate(Item item) {
        if (item == Items.GOLDEN_APPLE) {return ov4Utils::isGapple;}
    ...
    private Item getItem() {
        ...
        switch (itemMode.get()) {
            ...
            case Gapple -> {
                if (itemAvailable(ov4Utils::isGapple)) {
                    return Items.GOLDEN_APPLE;
                }
            }
            case Bed -> {
