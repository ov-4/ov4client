...
public class PistonCrystal extends ov4Module {
    public PistonCrystal() {
        super(ov4client.Combat, "Piston Crystal", "Pushes crystals into your enemies to deal massive damage.");
    }
        ...
        for (int x = (crystalDir.getOpposite().getOffsetX() == 0 ? -1 : Math.min(0, crystalDir.getOffsetX())); x <= (crystalDir.getOpposite().getOffsetX() == 0 ? 1 : Math.max(0, crystalDir.getOpposite().getOffsetX())); x++) {
            for (int y = 0; y <= 1; y++) {
                for (int z = (crystalDir.getOpposite().getOffsetZ() == 0 ? -1 : Math.min(0, crystalDir.getOffsetZ())); z <= (crystalDir.getOpposite().getOffsetZ() == 0 ? 1 : Math.max(0, crystalDir.getOpposite().getOffsetZ())); z++) {
                    ...
                    if (!ov4Utils.solid(pos.down())) continue;
    ...
    private void redstonePos(BlockPos pos, Direction pDir, BlockPos cPos) {
        ...
        if (redstone.get() == Redstone.Torch) {
            for (Direction direction : Direction.values()) {
                ...
                if (!ov4Utils.replaceable(position) && !(mc.world.getBlockState(position).getBlock() instanceof RedstoneTorchBlock) && !(mc.world.getBlockState(position).getBlock() instanceof FireBlock)) {
                    continue;
                }
        ...
        for (Direction direction : Direction.values()) {
            ...
            if (Box.from(new BlockBox(position)).intersects(ov4Utils.getCrystalBox(cPos))) {
                continue;
            }
