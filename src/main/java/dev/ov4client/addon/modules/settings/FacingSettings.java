...
public class FacingSettings extends ov4Module {
    public FacingSettings() {
        super(ov4client.Settings, "Facing", "Global facing settings for every ov4 module.");
    }
        ...
        if (mc.world != null && mc.player != null) {
            if (airPlace.get()) {
                ...
                for (Direction dir : Direction.values()) {
                    ...
                    if (!ov4Utils.solid(pos.offset(dir)) && (predicate != null && !predicate.test(pos.offset(dir)))) {
                        continue;
                    }

                    // Strict dir check (checks if face is on opposite side of the block to player)
                    if (strictDir.get() && !ov4Utils.strictDir(pos.offset(dir), dir.getOpposite())) {
                        continue;
                    }
        ...
        if (mc.world != null && mc.player != null) {
            if (airPlace.get()) {
                ...
                for (Direction dir : Direction.values()) {
                    ...
                    // Test if there is block in the side and if predicate is valid
                    if (!ov4Utils.solid(pos.offset(dir)) || (predicate != null && !predicate.test(dir)) || (predicatePos != null && !predicatePos.test(pos.offset(dir)))) {
                        continue;
                    }
        ...
        if (mc.world != null && mc.player != null) {
            if (airPlace.get()) {
                ...
                for (Direction dir : Direction.values()) {
                    ...
                    // Test if there is block in the side and if predicate is valid
                    if (!ov4Utils.solid(pos.offset(dir))) {
                        continue;
                    }
    ...
    public Direction getPlaceOnDirection(BlockPos pos) {
        ...
        if (mc.world != null && mc.player != null) {
            ...
            for (Direction dir : Direction.values()) {
                ...
                if (strictDir.get() && !ov4Utils.strictDir(pos, dir)) {
                    continue;
                }
