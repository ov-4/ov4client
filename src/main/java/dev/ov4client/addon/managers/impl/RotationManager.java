...
import dev.ov4client.addon.modules.settings.RotationSettings;
import dev.ov4client.addon.utils.ov4Utils;
import dev.ov4client.addon.utils.SettingUtils;
import dev.ov4client.addon.utils.player.NCPRaytracer;
...
public class RotationManager {
    ...
    public boolean start(BlockPos pos, Box box, Vec3d vec, double p, RotationType type, long key) {
        ...
        if (p < priority || (p == priority && (!(target instanceof BoxTarget) || SettingUtils.rotationCheck(((BoxTarget) target).box, type)))) {
            ...
            this.key = key;
            target = pos != null ? new BoxTarget(pos, vec != null ? vec : ov4Utils.getMiddle(box), p, type) : new BoxTarget(box, vec != null ? vec : ov4Utils.getMiddle(box), p, type);
            timer = settings.time(type);
        }
